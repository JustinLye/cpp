...
void jl::save(void* data, std::size_t size, const char* filename) {
	std::FILE* out = std::fopen(filename, "wb");
	std::fwrite(data, size, 1, out);
	std::fclose(out);
}
void jl::load(void* data, std::size_t size, const char* filename) {
	std::FILE* in = std::fopen(filename, "rb");
	std::fread(data, size, 1, in);
	std::fclose(in);
}
const char* jl::int_to_binary(std::size_t a) {
